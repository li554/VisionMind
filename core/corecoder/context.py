...
def _approx_tokens(content) -> int:
    """Rough token count. For str: ~3 chars/token; for multimodal list:
    text parts by length, image parts estimated as fixed cost."""
...
class ContextManager:
    ...
    def _flatten(messages: list[dict]) -> str:
        ...
        for m in messages:
            ...
            if text:
                parts.append(f"[{role}] {ContextManager._content_preview(text)}")
        return "\n".join(parts)

    @staticmethod
    def _content_preview(content) -> str:
        """多模态 content list → 一行文本预览(图片部件转占位符)"""
        if isinstance(content, str):
            return content[:400]
        if isinstance(content, list):
            previews = []
            for part in content:
                if not isinstance(part, dict):
                    continue
                if part.get("type") == "text":
                    previews.append(part.get("text", "")[:400])
                elif part.get("type") == "image_url":
                    previews.append("[图片]")
            return " | ".join(previews)[:400]
    ...
    def _extract_key_info(messages: list[dict]) -> str:
        ...
        for m in messages:
            ...
            text = content if isinstance(content, str) else ContextManager._content_preview(content)
            # extract file paths
